//esta clase es para que el profesional pueda editar su perfil
...
function editarCampo(campo, etiqueta, tipo) {
    ...
    if (tipo === 'number') { // Si es número, establecer rango
        ...
        document.getElementById('modalValor').max = 70;
    }
...
document.getElementById('inputLogo').addEventListener('change', function(e) { //change es un evento que se dispara cuando el valor de un elemento cambia
    const file = e.target.files[0]; //.target es el elemento que disparó el evento, .files es una lista de archivos seleccionados
    if (file) {
        // Validar tamaño (máximo 2MB)
        if (file.size > 2 * 1024 * 1024) {
            alert('La imagen es muy grande. Máximo 2MB.');
        ...
        const reader = new FileReader();// FileReader es una API para leer archivos del cliente
        reader.onload = function(e) {//.onload es un evento que se dispara cuando la lectura del archivo se completa
            const imageUrl = e.target.result;
            logoTemporal = imageUrl;
